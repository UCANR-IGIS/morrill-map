---
title: "California Morril Map: Acreage Sold to Establish UC, by Tribe"
...
This report calculates the amount of land that was used to support the creation of the University of California under the [1862 Morrill Act](https://en.wikipedia.org/wiki/Morrill_Land-Grant_Acts){target="_blank" rel="noopener"}, by Tribe.
...
Load the indigenous territories that intersect California ([data source](native-land-prep.R){target="_blank" rel="noopener"}):
...
# 3. Parcels Claimed by the State of California by Tribe

Here we summarize the number and acreage of parcels that were claimed by agents of the State of California to establish UC, by Tribe. Note that due to overlap between Indigenous Territories polygons, some parcels are double-counted in this table.
...
```{r}
## Contrust the names of the tribes for each parcle
...
m <- leaflet(uc_prcl_ge_pdt_sf) %>%
  ...
  hideGroup("Indigenous Territories")
...
The following language for a land acknowledgment aims to highlight the role the 1862 Morrill Act played in transforming land that had been seized from Native Americans through [centuries of violence](https://yalebooks.yale.edu/book/9780300181364/american-genocide){target="_blank" rel="noopener"} by Spaniards, Mexicans, and Euro-Americans into an endowment for the nascent University of California. 
...
<p style="color:crimson;">The University of California was established in part with the proceeds from land seized from the <em>`r paste(tribe_names, collapse=", ")`</em> tribes. Details.</p>
